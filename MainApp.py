...
class MainApp:
    ...
    def act_randomtopo(self):
        ...
        self.frame_left_label.pack(side=TOP, fill=X)
        self.frame_left_top = Frame(self.control_frame_left, borderwidth=2, relief=RIDGE)
        Label(self.frame_left_top, text="Number of nodes:").grid(row=0, sticky=W)
        ...
        self.node_num_rand.insert(END, '4')
        ...
        b_ok = Button(self.frame_left_top, text="OK", width=6,
                      ...
                      relief=GROOVE).grid(row = 2, column = 0, columnspan = 2)
        self.frame_left_top.place(height=93, width=180, x=5, y=150)

        self.frame_left_bottom = Frame(self.control_frame_left, borderwidth=2, relief=RIDGE)
        # self.b5 = Button(self.frame_left_bottom, text="open", width=10, command=self.callback_open, relief=GROOVE)
        # self.b5.pack(side=TOP, padx=2, pady=2)
        self.b6 = Button(self.frame_left_bottom, text="save", width=10, command=self.callback_save, relief=GROOVE)
    ...
    def callback_mesh_prot(self):
        ...
        for src in graph:
            for dst in graph:
                if src < dst and (src, dst) in Set_Links:
                    all_path = []
        ...
        for l in Link_ID:
            for i in Num_cand[l]:
                for j in range(0, len(Route_Table[l][i]) - 1):
                    ...
                    indx = Set_Links.index(tuple(sorted((m, n))))
                    y[(l, i, indx)] = 1

        prob = LpProblem('mesh_protection', LpMinimize)
        S = LpVariable.dicts('S', (Link_ID), 0, None, LpInteger)
        P = LpVariable.dicts('P', (Link_ID, TED), 0, None, LpInteger)

        ####objective function
        prob += lpSum(S[l] for l in Link_ID)
        # constraint 1
        for l in Link_ID:
            prob += lpSum(P[l][i] for i in Num_cand[l]) == L[Set_Links[l]]

        # constraint 2
        for l in Link_ID:
            for l1 in Link_ID:
                prob += S[l1] - lpSum(P[l][i] * y[(l, i, l1)] for i in Num_cand[l]) >= 0
        ...
        for key in self.edge_list:
            ...
            if node1 < node2:
                l = Set_Links.index((node1,node2))
            else:
                l = Set_Links.index((node2,node1))
            coor1 = self.node_pos[node1]
            coor2 = self.node_pos[node2]
            midx1 = (coor1[0] + coor1[2]) / 2.0
            midy1 = (coor1[1] + coor1[3]) / 2.0
    ...
    def callback_test_rand(self):
        ...
        while len(self.node_list) < self.node_num:
            self.click_x = random.randint(1, 500)
            self.click_y = random.randint(1, 300)
        ...
        while deg_limit < 1:
            for node in candidate_nodes:
                if node_degree[node] <= deg_limit+1:
                    print "node", node
                    distance[node]={}
                    # calculate distance
                    for neib_node in candidate_nodes:
                        print "neib node", neib_node
                        if neib_node != node:
                            if neib_node not in topo[node]:
                                if node_degree[neib_node] <= deg_limit+1:
                    ...
                    if distance[node]:
                        ...
                        self.edge = self.canvas.create_line(prev_x, prev_y,
                                                    choosen_x, choosen_y, fill=self.defaultLineColor, arrow=None,
                                                    width=3)
                        self.edge_list[self.edge] = {}
                        self.edge_list[self.edge]['fromto'] = (node, choosen_neib)
                        self.edge_list[self.edge]['distance'] = {}
                        self.edge_list[self.edge]['distance'][self.node] = float('0')
                        topo[node].append(choosen_neib)
                        topo[choosen_neib].append(node)
                        print topo
            deg_limit+=1
