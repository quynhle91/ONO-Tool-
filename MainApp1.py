...
class MainApp:
   ...
   def __init__(self, parent):
      ...
      self.f1 = Button(self.functionbar, text="shortest path", width=10, command=self.callback_shortestPath,relief=GROOVE)
      ...
      self.f2 = Button(self.functionbar, text="partitioning graph", width=10, command=self.callback_partitioning, relief=GROOVE)
      self.f2.pack(side=LEFT, padx=2, pady=2)
   ...
   def callback_partitioning(self):
      ...
      for i in range(0,len(shared_routes)):
         edges_subnets[i] = {}
         edges_subnets[i][0] = tuple(shared_routes[i])
         edges_subnets[i][1] = tuple(shared_routes[i])
         m = self.set_border_nodes.index(shared_routes[i][0])
         n = self.set_border_nodes.index(shared_routes[i][-1])
         #while shared_routes[i][0] = self.set_border_nodes[n]
      ...
      #          edges_subnets[i][0] = edges_subnets[i][0] + (self.set_border_nodes[j],)
      #       gen = (k for k in self.set_border_nodes if k not in edges_subnets[i][0])
      #       for k in gen:
      #          edges_subnets[i][1] = edges_subnets[i][1] + (k,)
      #       #print 'edges_subnets[i][1]', edges_subnets[i][1]
      #    else:
      #       #print 'check n<m'
      #       for j in range(n+1, m):
      #          #if self.set_border_nodes[j] not in edges_subnets[i][0]:
      #          edges_subnets[i][0]= edges_subnets[i][0]+(self.set_border_nodes[j],)
      #       #print 'edges_subnets[i][0]',edges_subnets[i][0]
      #       gen = (k for k in self.set_border_nodes if k not in edges_subnets[i][0])
      #       for k in gen:
      #          print 'shared_routes',shared_routes[i]
      #          edges_subnets[i][1]=edges_subnets[i][1]+(k,)
      #       #print 'edges_subnets[i][1]', edges_subnets[i][1]
      # print 'edges_subnets',edges_subnets
...
##      print 'self.graph',self.graph
      ...
      #print 'My',self.first_node
      #set_border_nodes.append(self.first_node)
      #self.canvas.itemconfig(self.first_node,fill=color)
      # for k in self.node_cor.keys():
      #    self.xy = self.canvas.create_text(self.node_cor[k][0]+15,self.node_cor[k][1],
      #                                      text='x,y='+str(self.node_cor[k]))
...
##         self.angle=self.canvas.create_text(self.node_cor[n][0],self.node_cor[n][1]+30,text='angle='+str(angle))
      ...
      while chosen_node != self.first_node:   
         ...
         #self.vect=self.canvas.create_text(chosen_node_cor[0],chosen_node_cor[1]+45,text='ref_vect='+str(ref_vec))

         angle_ref = math.atan(ref_vec[1]/ref_vec[0])*180/math.pi
         #self.angle=self.canvas.create_text(chosen_node_cor[0],chosen_node_cor[1]+60,text='angle_ref='+str(angle_ref),fill = 'green')
         if ref_vec[0]<0 and ref_vec[1]<0:
            ...
            ref_quad = 3
         #self.vect = self.canvas.create_text(chosen_node_cor[0], chosen_node_cor[1] + 45,
         #                                   text='ref_vect=' + str(ref_vec)+'  ref quad'+str(ref_quad))
#         pref_quad = [ref_quad%4,(ref_quad+1)%4,(ref_quad+2)%4,(ref_quad+3)%4]
         new_chosen_node = -5
         new_chosen_node_prio = 5
         new_chosen_node_angle = 91
         gen = (n for n in self.graph[chosen_node] if n not in set_border_nodes)
         for n in gen:
            #if n not in set_border_nodes:

               tmp_node_cor = self.node_cor[n]
               temp_vec = [tmp_node_cor[0]-chosen_node_cor[0],tmp_node_cor[1]-chosen_node_cor[1]]
               if temp_vec[0] == ref_vec[0] and temp_vec[1] == ref_vec[1]:
                  print 'skip case'
               else:
                  #self.canvas.itemconfig(n, fill=color)
                  #self.vect=self.canvas.create_text(tmp_node_cor[0],tmp_node_cor[1]+30,text='vect='+str(B),fill = color)
                  temp_angle = math.atan(temp_vec[1]/temp_vec[0])*180/math.pi
                  #self.angle=self.canvas.create_text(self.node_cor[n][0],self.node_cor[n][1]+60,text='angle_temp='+str(temp_angle),fill=color)
                  temp_angle = abs(temp_angle)
                  angle_vec = math.acos((ref_vec[0]*temp_vec[0]+ref_vec[1]*temp_vec[1])/(math.hypot(ref_vec[0],ref_vec[1])*math.hypot(temp_vec[0],temp_vec[1])))*180/math.pi
                  #self.vect=self.canvas.create_text(tmp_node_cor[0],tmp_node_cor[1]+30,text='angle_vec='+str(angle_vec)+str(temp_quad),fill = color)
                  if temp_vec[0]<0 and temp_vec[1]<0:
                     temp_quad = 0
                  elif temp_vec[0]>0 and temp_vec[1]<0:
                     temp_quad = 1
                  elif temp_vec[0]>0 and temp_vec[1]>0:
                     temp_quad = 2
                  elif temp_vec[0]<0 and temp_vec[1]>0:
                     temp_quad = 3
                  #self.vect = self.canvas.create_text(tmp_node_cor[0], tmp_node_cor[1] + 30,
                  #                                   text='angle_vec=' + str(angle_vec) +'temp quadrant'+ str(temp_quad), fill=color)

                  if ref_quad == 0:
                     print 'Ref Quad = 0'
                     if temp_quad == 0:
                        print 'Temp Quad = 0'
                        if temp_angle > angle_ref:
                           #print 'Temp is in zone 0'
                           temp_prio = 0
                        else:
                           #print 'Temp is in zone 4'
                           temp_prio = 4

                        if new_chosen_node_angle == 91:
                           #print 'First temp considered'
                           new_chosen_node = n
                           new_chosen_node_prio = temp_prio
                           new_chosen_node_angle = temp_angle
                        else:
                           if new_chosen_node_prio>temp_prio:
                              #print 'Temp is in higher prior zone'
                              new_chosen_node = n
                              new_chosen_node_prio = temp_prio
                              new_chosen_node_angle = temp_angle
                           elif new_chosen_node_prio==temp_prio:
                              #print 'Temp is in the same priority zone'
                              if temp_prio==0:
                                 #print 'Zone 0'
                                 if new_chosen_node_angle>temp_angle:
                                    #print 'Temp is chosen zone 0.'
                                    new_chosen_node = n
                                    new_chosen_node_prio = temp_prio
                                    new_chosen_node_angle = temp_angle
                              elif temp_prio == 4:
                        ...
                        if new_chosen_node_prio>temp_prio:
                           ...
                           if new_chosen_node_angle<temp_angle or new_chosen_node_angle==91:
                              new_chosen_node = n
                              new_chosen_node_prio = temp_prio
                              new_chosen_node_angle = temp_angle
                        ...
                        temp_prio = 2
                        if new_chosen_node_prio>temp_prio:
                           #print 'went through temquad 2'
                           new_chosen_node = n
                           new_chosen_node_prio = temp_prio
                           new_chosen_node_angle = temp_angle
                        elif new_chosen_node_prio==temp_prio:
                           #print 'check new prio = temp prio'
                           if new_chosen_node_angle>temp_angle or new_chosen_node_angle==91:
                              new_chosen_node = n
                              new_chosen_node_prio = temp_prio
                              new_chosen_node_angle = temp_angle
                        ...
                        if new_chosen_node_prio>temp_prio:
                         ...
                         print "Error temp has no quadrant"
...
#######################################################################################
                  ...
                  #print 'New Chosen = ', new_chosen_node
         #print 'Out_New Chosen = ', new_chosen_node
   ...
   def callback_clear(self):
      if self.node_list or self.edge_list or self.topo or self.node_pos:
         if tkMessageBox.askokcancel("Warning!", "Do you really want to clear workspace?\nAll items will be removed!!!"):
            #----- reset canvas & button ----#
               ...
               if len(tmp_node_list): #create node
                  ...
                  for inode in tmp_node_list.keys():
                     ...
                     self.text = self.canvas.create_text((coor1[0]+coor1[2])/2.0,
                                ((coor1[1]+coor1[3])/2.0)+25, text=name) #create node label
                  ...
                  for iedge in tmp_edge_list: #create edge
                     ...
                     self.text = self.canvas.create_text((midx1+midx2)/2.0+10,
                                 (midy1+midy2)/2.0-10, text=distance) #create edge label
   ...
   def callback_showtrf(self):
      ...
      Label(self.frame_link_info_top, text="Source(comma)Destination(comma)Distance(comma)(newline)").grid(row=1, sticky=W)
      Label(self.frame_link_info_top, text="-------------------------------------------------------").grid(row=2, sticky=W)
      ...
      Label(self.frame_link_info_top, text="-------------------------------------------------------").grid(row=7, sticky=W)
   ...
   def callback_opt_optimize(self):
      ...
      if self.topo and self.traffic_mat: #NOTE:  Need to check traffic matrix for consistency. (i.e., node name)
         (self.paths_allo_bw, self.wave_link,self.add_wave_link, self.obj_value) = optim.opt_wave(self.topo,\
          self.k_num, self.traffic_mat, self.wave_num, self.fiber_num, self.slot_num,self.obj_cri,2-self.trf_split)
         ...
         if self.paths_allo_bw and self.wave_link:
            ...
            if self.obj_cri == 1:
               ...
               Label(self.frame_link_info_topmost, text="(Minimize resource)\n"+"Total number of utilized wavelength channels = "+\
                     str(self.obj_value),fg="blue").grid(row=0)
            if self.obj_cri == 2:
               self.reset_frame_info_bottom() #reset info frame
               self.frame_link_info_topmost = Frame(self.scrollframe_info_bottom)
               self.frame_link_info_topmost.pack(side=TOP, expand=YES, fill=BOTH)
               Label(self.frame_link_info_topmost, text="(Maximize supported traffic units)\n"+"Total number of supported traffic units = "+\
                     str(self.obj_value),fg="blue").grid(row=0)
         else:
         ...
         if self.add_wave_link:
            ...
            self.scrollframe_top_opt=ScrolledFrame(self.frame_add_wave,370,200) #Scrolled additional wav needed
   ...
   def callback_select(self,event):
      ...
      oval_list = [item for item in self.canvas.find_overlapping(self.click_x-5, self.click_y-5, self.click_x+5, self.click_y+5) if self.canvas.type(item) == "oval"]
      line_list = [item for item in self.canvas.find_overlapping(self.click_x-5, self.click_y-5, self.click_x+5, self.click_y+5) if self.canvas.type(item) == "line"]
      if len(oval_list):
      ...
      if self.sel_item and self.sel_type == "oval": 
         ...
         #create edge or finding shortest path (click on the first node)
            ...
            if self.do == "create edge":
               ...
               self.status.set(self.status_mode+": "+ nodename_select + " is not a source node in the traffic file.")
               return
            if self.do != "optimize":
               self.canvas.itemconfig(self.sel_item,fill="red")
            ...
            if self.prev_item == self.sel_item:
               ...
               if not self.topo.has_key(self.node_list[self.sel_item]['name'].values()[0]):
                  ...
                  if self.topo[self.node_list[self.sel_item]['name'].values()[0]].has_key(self.node_list[self.prev_item]['name'].values()[0]):
               ...
               if create_line: #CREATE LINE /EDGE
                  ...
                  self.edge = self.canvas.create_line(self.prev_x, self.prev_y,
                                                      curr_x, curr_y, fill=self.defaultLineColor, arrow=None, width=3)
                  ...
                  print "line created ", self.prev_x,self.prev_y,(self.sel_coor[0]
                    +self.sel_coor[2])/2.0,(self.sel_coor[1]+self.sel_coor[3])/2.0
         ...
         self.node = self.canvas.create_oval(self.click_x-15,self.click_y-15,self.click_x+15,self.click_y+15, fill="CYAN")
               ...
               if self.sel_item in self.edge_list[key]['fromto']:
                  ...
                  if midx2 - midx1 != 0: #text position
                     if (midy2 - midy1)/(midx2 - midx1) >= 0:
                        self.canvas.coords(distance_id,(midx1+midx2)/2.0-15,(midy1+midy2)/2.0+15) #move name
                     else:
                        self.canvas.coords(distance_id,(midx1+midx2)/2.0+15,(midy1+midy2)/2.0+15) #move name
                  else:
                     self.canvas.coords(distance_id,(midx1+midx2)/2.0+15,(midy1+midy2)/2.0) #move name
   ...
   def draw_path(self): #SHORTEST PATH
               ...
               for key in self.edge_list:
                  ...
                  if node1 in self.node_list[key1]['name'].values() and \
                     ...
                     self.draw_path_key.append(self.canvas.create_line(self.canvas.coords(key), \
                                               fill=color[ipath-1], arrow=None, width=size[ipath-1]))
      ...
      text = self.info_canvas.create_text(100+100, 20, text=self.paths[1]['link_list'][0][0]+\
                                        " to "+self.paths[1]['link_list'][len(self.paths[1]['link_list'])-1][1])
      ...
      for i in range(len(color)):
         ...
         text2 = self.info_canvas.create_text(200+40, 100+i*20, text=": "+str(self.paths[i+1]['dist']) + " km")
      ...
      if not self.paths.has_key('link_list'):
         ...
         for ipath in self.paths:
            ...
            edge = self.info_canvas.create_line(20, 103+icolor*20, 
                           90, 103+icolor*20, fill=color[icolor], arrow=None, width=size[icolor])
            print 'create line'
            text = self.info_canvas.create_text(130+40, 100+icolor*20, text="Allocated bandwidth (unit):  "+str(bandwidth))
            for [node1,node2] in self.paths[ipath]['link_list']:
               for key in self.edge_list:
                  key1 = self.edge_list[key]['fromto'][0]
                  key2 = self.edge_list[key]['fromto'][1]
                  if node1 in self.node_list[key1]['name'].values() and \
                     ...
                     self.drawopt_link_key[self.index].append(self.canvas.create_line(self.canvas.coords(key), \
                                               fill=color[icolor], arrow=None, width=size[icolor]))
               format_path.append('('+node1+','+node2+')')
            self.paths[ipath]['link_list'] = format_path
            icolor += 1
         self.info_canvas.pack(side=LEFT, expand=YES, fill=BOTH)
            ...
            if (link1[0] in node1 and link1[1] in node2) or (link1[0] in node2 and link1[1] in node1):
               if text_key in text_key_add:
                  ...
                  self.draw_addwave_key.append(self.canvas.create_text([coorx+10,corry],\
                                         text=" , "+str(self.add_wave_link[link]),fill="red"))
               else:
   ...
   def add_legend(self): #LEGEND
      ...
      for i in range(len(icolor)):
         edge = self.canvas.create_line(700, 55+i*20, 725, 55+i*20, fill=icolor[i], arrow=None, width=isize[i])
...
#--------------------------------------------------------------------#
   ...
   def show_common(self):
      ...
      name2 = self.node_list[node2]['name'].values()[0]
      if self.do == "optimize":
         self.labelText = ["distance (km)",name1+"-"+name2,"Wavelength Utilization","Additional Wavelengths Needed"]
         ...
         if self.wave_link_use:
            if not self.wave_link_use[name1][name2]:
               ...
               self.dialogText.append(str(100.0*self.wave_link_use[name1][name2]/self.wave_num[name1][name2])+'%')
         else:
            self.dialogText.append('-')
         if self.add_wave_link:
         ...
         for nName in self.labelText:  
            if i == 1:
               ...
               Label(self.frame_link_info_top, text=self.dialogText[self.labelText.index(nName)]).grid(row=i,column=2)
            i += 1
         ...
         if self.wave_link_use:
            if not self.wave_link_use[name2][name1]:
               ...
               self.dialogText.append(str(100.0*self.wave_link_use[name2][name1]/self.wave_num[name2][name1])+'%')
         else:
            self.dialogText.append('-')
         if self.add_wave_link:
         ...
         for nName in self.labelText:
            if i > 1:
               ...
               Label(self.frame_link_info_bottom, text=self.dialogText[self.labelText.index(nName)]).grid(row=i,column=2)
            else:
      ...
      # END OPTIMIZATION
         ...
         self.labelText = ["distance (km)",name1+"-"+name2,"No. of fibers","capacity (wavelengths/fiber)",\
                           "capacity (slots/wavelength)","Wavelength Utilization"]
         distance = self.edge_list[self.sel_item]['distance'][distance_id]
         self.dialogText = [distance,distance]
         self.dialogText.append(self.fiber_num[name1][name2])
         self.dialogText.append(self.wave_num[name1][name2])
         ...
         for nName in self.labelText:  
            ...
            i += 1
         self.labelText = [name2+"-"+name1,"No. of fibers","capacity (wavelengths/fiber)",\
                           "capacity (slots/fiber)","Wavelength Utilization"]
         self.dialogText = [distance]
         self.dialogText.append(self.fiber_num[name2][name1])
         self.dialogText.append(self.wave_num[name2][name1])
         self.dialogText.append(self.slot_num[name2][name1])
   ...
   def opt_show_top_frame(self,choice):
      ...
      if self.filename:
         ...
         self.b1_opt = Button(self.toolbar_opt, text="run", width=6, command=self.callback_opt_optimize,relief=GROOVE)
         ...
         self.b2_opt = Button(self.toolbar_opt, text="edit traffic file", width=10, command=self.callback_edittrf,relief=GROOVE)
         self.b2_opt.pack(side=LEFT, padx=2, pady=2)
         
         choice_opt = [('Minimize number of utilized wavelength channels',1), ('Maximize number of supported traffic units',2), \
            ...
            if len(sline) == 4:
               ...
               if self.s[i].get() in name_list and self.d[i].get() in name_list:
                  ...
                  Radiobutton(self.frame_opt, text='', value=i-1, \
                           variable=self.var_showpath, state=NORMAL,command=self.show_path_opt).grid(row=i,column=3)#added 8/25/2011
         ...
         if err_count:
            tkMessageBox.showwarning("Error detected!!!", "Discard " + str(err_count) + " demand(s)\check nodes' name") #show info
